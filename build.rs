...
fn main() -> Result<(), std::io::Error> {
    ...
    let bin_name = env::var("CARGO_BIN_NAME").unwrap_or_else(|_| "app".to_string());
    File::create(out_dir.join("bin-name.txt"))?.write_all(bin_name.as_bytes())?;
...
fn commit_hash() -> Result<String, Ignore> {
    ...
    Ok(String::from_utf8(
        Command::new("git").args(args).output()?.stdout,
    )?)
}
